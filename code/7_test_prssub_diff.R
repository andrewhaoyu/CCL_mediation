...
ind = 100
result_vec = rep(0,ind)
for(k in 1:ind){
  #shuffle y
  shuffled_indices <- sample(nrow(data_com))
  shuffled_time <- data_com$y[shuffled_indices]
  shuffled_status <- data_com$case_control_cancer_ignore[shuffled_indices]
  data_com$y = shuffled_time
  data_com$case_control_cancer_ignore = shuffled_status
...
save(result_vec, file = paste0("./result/mediation_result_permu_",i1,"_",i2,".rdata"))
